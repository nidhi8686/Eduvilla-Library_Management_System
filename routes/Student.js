            ...
            db.query(getlimitedData, (err, result2) => {
                ...
                db.query(getbookData, (err, result3) => {
                    ...
                    db.query(getissuedbooks, [session[0].email, "pending"], (err, result4) => {
                        if (err) throw err;
                        res.render('Pages/Student/Dashboard.ejs', {
                            user: session,
                            bookdata: result2,
                            lastaddedbooks: result3,
                            issuedbooks: result4
                        });
                    })
                })
...
router.get('/bookissue/:bookid', (req, res) => {
    if (req.session.user) {
        ...
        db.query(getData, [bookid], (err, result) => {
            if (err) throw err;
            res.render('Pages/Student/bookissue.ejs', {
                bookdata: result,
                user: session
            });
        })
...
router.get('/mybooks', (req, res) => {
    if (req.session.user) {
        ...
        db.query(getData, [session[0].email, "pending"], (err, result) => {
            if (err) throw err;
            res.render('Pages/Student/mybooks.ejs', {
                user: session,
                issuedbooks: result
            })
        })
...
router.get('/summary', (req, res) => {
    if (req.session.user) {
        ...
        db.query(getData, [session[0].email], (err, result) => {
            if (err) throw err;
            res.render('Pages/Student/summary.ejs', {
                user: session,
                issuedbooks: result
            })
        })
...
router.get('/fine', (req, res) => {
    if (req.session.user) {
        ...
        db.query(getData, [session[0].email], (err, result) => {
            if (err) throw err;
            res.render('Pages/Student/fine.ejs', {
                user: session,
                issuedbooks: result
            })
        })
